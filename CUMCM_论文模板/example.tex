...
\title{2026 全国大学生数学建模竞赛 \LaTeX{} 模板}
\tihao{A}
...
\begin{document}
 ...
 \maketitle
 \begin{abstract}
cumcmthesis 是为全国大学生数学建模竞赛编写的 \LaTeX{} 模板，旨在让参赛者专注于论文内容。本示例按 2026 年论文格式规范更新，展示封面、摘要、正文、参考文献和附录的基本写法。正式提交前仍应核对全国组委会和所在赛区发布的最新通知。
...
innertopmargin=\topskip, frametitle={2026 年论文格式要点},
frametitlefont= \bfseries,frametitlerule=true,frametitlealignment =\raggedright\noindent,
frametitlerulewidth=.35pt, frametitlebackgroundcolor=gray!2,]
根据全国组委会发布的《全国大学生数学建模竞赛论文格式规范（2026 年修订稿）》，应重点检查：
\begin{enumerate}
\item 纸质版第一页为承诺书、第二页为编号专用页、第三页为摘要专用页；摘要原则上不超过一页，并从该页开始以阿拉伯数字连续编页码。
\item 正文从第四页开始，\textbf{\color{red}不要目录}，正文不超过 30 页；正文之后为页数不限的附录。
\item 电子版论文为单独的 PDF 或 Word 文件，大小不超过 20 MB，且不得包含承诺书和编号专用页。
\item 摘要、正文、附录和支撑材料不得出现参赛者身份、学校或赛区信息；附录应列出支撑材料文件并包含完整、可运行的源程序。
\end{enumerate}

\end{mdframed}


\noindent 2026 年官方格式规范见\href{https://www.mcm.edu.cn/html_cn/node/4cd596519c9eb9fbd866398f6df0caa3.html}{全国大学生数学建模竞赛官网}。
...
建议安装当前维护中的 \TeX{} Live 或 MiK\TeX{}，并使用支持 UTF-8 与 XeLaTeX 的编辑器。
...
table 环境是一个将表格嵌入文本的浮动环境。tabular 环境的必选参数由每列对应一个格式字符所组成：c 表示居中，l 表示左对齐，r 表示右对齐，其总个数应与表的列数相同。此外，\verb|@{文本}| 可以出现在任意两个上述的列格式之间，其中的文本将被插入每一行的同一位置。表格的各行以 \verb|\\| 分隔，同一行的各列则以 \& 分隔。\verb|\toprule|、\verb|\midrule| 和 \verb|\bottomrule| 由 booktabs 宏包提供。本模板示例将上下线设为 0.8pt、中线设为 0.5pt，避免出现过粗边界。
...
以上仅简单介绍了基础用法。对于更复杂的需求，可在命令行运行 \verb|texdoc amsmath| 阅读宏包手册；数学符号可通过 \verb|texdoc symbols-a4| 查阅。
...
正式论文应在正文引用处使用 \verb|\cite{bibkey}| 标注文献。例如，2026 年论文格式要求可引用全国组委会发布的规范\cite{mathematical-modeling}。
...
\section*{人工智能工具使用声明}
请依据全国组委会在竞赛期间施行的人工智能工具使用规定填写本节；未使用时也应按规定作出明确声明。
...
\section{支撑材料文件列表}
请在此列出全部支撑材料文件及其用途。若没有支撑材料，应按 2026 年规范明确说明“本论文没有支撑材料”。
