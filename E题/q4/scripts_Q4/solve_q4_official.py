...
ROOT=r'C:/Users/ZhangChaobo/Desktop/CUMCM2026Problems/E题'; OUT=os.environ.get('Q4_OUT',ROOT+'/q4/data_Q4_rev'); os.makedirs(OUT,exist_ok=True)
s1=pd.read_csv(ROOT+'/clean_data/sheet1_投放记录_clean.csv',encoding='utf-8-sig'); s2=pd.read_csv(ROOT+'/clean_data/sheet2_日注册_clean.csv',encoding='utf-8-sig'); s3=pd.read_csv(ROOT+'/clean_data/sheet3_关键词_clean.csv',encoding='utf-8-sig')
...
# 直接读取问题二最终分类。
cl=pd.read_excel(ROOT+'/q2/data_Q2/result2.xlsx'); long=cl.melt(id_vars=['方案ID','推广单元','序号'],value_vars=['黄金词','重点词','潜力词','问题词','无效词'],var_name='类型',value_name='关键词').dropna();long.关键词=long.关键词.astype(int)
s3=long.merge(s3,left_on=['方案ID','推广单元','关键词'],right_on=['方案ID','推广单元ID','关键词ID'],how='inner'); active=(s3.消费额>0)&(s3.点击量>0)
